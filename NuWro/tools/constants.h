...
namespace Constants{
 ...
 //Muon
  ...
  int muon_ylim[num_runs][num_var] = {{40,160,100}, //jan
				      ...
				      {1700,2000,1450}}; //runs 1+2+3 xsec binning
 ...
 //Leading Proton
  ...
  int leading_ylim[num_runs][num_var] = {{35,160,100},//jan
					 ...
					 {2000,1200,1500}};//runs 1+2+3 xsec binning
  ...
  int recoil_ylim[num_runs][num_var] = {{60,80,150},//jan
					...
					{2200,1200,1300}};//runs 1+2+3 xsec binning
 ...
 ///////////////////////////
  static const int num_phys = 3; //NOTE: These values are the same for Raquel's plots. If this changes in the future, be sure to chage this!
  const char* physics[num_phys] = {"_opening_angle_protons_lab","_opening_angle_mu_both","_nu_E"};//,"_PT_squared","_mom_struck_nuc","_tot_pz","_tot_E","_tot_E_minus_beam",};
  const char* physics_titles[num_phys] = {"cos(#gamma_{Lab})","cos(#gamma_{#mu,p_{L} + p_{R}})","Neutrino Energy (GeV/c)"};//,"P^{T}_{Miss}^{2} (GeV^{2}/c^{2})","Momentum of Struck Nucleon (GeV)","Total P_{z} of the Two Protons", "Total Kinetic Energy of System (GeV/c)","Total Kinetic Energy - Beam Energy (MeV/c)"};
  ...
  */
  ...
  int phys_ylim[num_runs][num_phys] = {{60,60,250},//jan
				       {150,300,1000},//run1
				       {250,500,1500},//run2
				       {250,800,1500},//run3
				       {400,1200,3500}, //runs 1+2+3 normal binning
				       {1200,1500,4000}}; //runs 1+2+3 xsec binning
  
  int phys_ymin[num_runs][num_phys] =  {{-2,-2,-2},//jan
					{-2,-2,-2},//run1
					{-2,-2,-2},//run2
					{-2,-2,-2},//run3
					{-2,-10,-2},//runs 1+2+3 normal binning
					{-2,-2,-2}};//runs 1+2+3 xsec binning

  double phys_bnb_ymin[num_runs][num_phys] =  {{2.7,2.7,2.7},//jan
					{2.7,2.7,2.7},//run1
					{2.7,2.7,2.7},//run2
					{2.7,2.7,2.7},//run3
					{2.7,2.7,2.7},//runs 1+2+3 normal binning
					{2.7,2.7,2.7}};//runs 1+2+3 xsec binning    
  ...
  int stv_ylim[num_runs][num_stv] = {{100,150,120,120},//jan
				     ...
				     {1200,3000,2000,1500}};//runs 1+2+3 xsec binning
 ...
 //////////////////////////
  ...
  static const int num_channels_raquel = 9;
  ...
  const char* channel_raquel[num_channels_raquel] = {"_total", "_ccMEC", "_ccDIS","_ccRES","_ccNue","_ccQE","_ccCOH","_nc","_outfv"};
  std::vector<const char*> channel_legend_raquel = {"Total Overlay","CCMEC","CCDIS","CCRES","CC#nu_{e}","CCQE","CCCOH","NC","Out of FV"};
