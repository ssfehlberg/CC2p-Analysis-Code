...
//Total covariance matrices are saved as TH2D inside of ../root_files/total_covariance_matrix_no_stat.root
//Plot images are saved inside of ../images/Total_Error/Covariance_Matrices
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
class total_covariance_matrix_no_stat{
 ...
 private:
  ...
  const char* file_titles[num_files] = {"GENIE Total","Detector Variations","Flux Multisims","G4 Multisims","Dirt (Sys.)"};
...
}; //end of total_covariance_matrix_no_stat class definition

void total_covariance_matrix_no_stat::main(){

  Grab_Histograms();

  //File where I will save the TH2Ds
  ///////////////////////////////////
  TFile *tfile_mine = new TFile(Form("../root_files/Total_Error/total_covariance_matrices_no_stat.root"),"RECREATE"); //output root file 
  ...
  for(int j=0; j < num_var; j++){
    ...
    plot_total_covariance_matrix(Form("_muon%s",var0[j]),Form("Total Covariance Matrix (no Stat.): Muon %s",var_titles[j]),muon_2D);
    plot_total_covariance_matrix(Form("_leading%s",var0[j]),Form("Total Covariance Matrix (no Stat.): Leading Proton %s",var_titles[j]),leading_2D);
    plot_total_covariance_matrix(Form("_recoil%s",var0[j]),Form("Total Covariance Matrix (no Stat.): Recoil Proton %s",var_titles[j]),recoil_2D);
  }
  ...
  for(int j=0; j < num_other_var; j++){
    ...
    plot_total_covariance_matrix(Form("%s",other_var[j]),Form("Total Covariance Matrix (no Stat.): %s",other_var_titles[j]),other_2D);
...
/////////////////////////////////////////////////////////////////
void total_covariance_matrix_no_stat::Grab_Histograms(){
...
/////////////////////////////////////////////
void total_covariance_matrix_no_stat::plot_total_covariance_matrix(const char* variable, const char* title, std::vector<TH2D*> hist_2D){
  ...
  hist->Write();
  canv->Print(Form("../images/Total_Error/Covariance_Matrices/total_covariance_matrix_no_stat%s.png",variable));
