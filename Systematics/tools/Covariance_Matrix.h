TH2D* make_covariance_matrix(TH1D* h_MC_tot, TH1D* h_EXT,TH1D* h_Dirt, std::vector<TH1D*> h_universe, double N_univ, const char* name,bool DIRT = false){
  ...
  for(int reco_bin = 1; reco_bin <nbins+1; reco_bin++){
    double EXT_reco = h_EXT->GetBinContent(reco_bin); //EXT content in reco bin i
    double Dirt_reco = h_Dirt->GetBinContent(reco_bin); //Dirt Content in reco bin i
    double MC_reco = (h_MC_tot->GetBinContent(reco_bin)); //Total Number of MC events in Reco Bin
    nu_CV[reco_bin-1] =  MC_reco + EXT_reco + Dirt_reco;
  }
  ...
  //If we are dealing with Dirt Systematics
  //////////////////////////////////////////
  if (DIRT == true){

    for(int reco_bin = 1; reco_bin <nbins+1; reco_bin++){
      double MC_reco = h_MC_tot->GetBinContent(reco_bin);
      double EXT_reco = h_EXT->GetBinContent(reco_bin);
      double Dirt_u_one_hundred = h_Dirt->GetBinContent(reco_bin);
      double Dirt_u_plus_30 = (1.3) * (h_Dirt->GetBinContent(reco_bin));

      nu_universe[reco_bin-1][0] = MC_reco + EXT_reco + Dirt_u_one_hundred;
      nu_universe[reco_bin-1][1] = MC_reco + EXT_reco + Dirt_u_plus_30;
    }
  

    //If we are looking at any other systematics
    ////////////////////////////////////////////
  }else{
	...
	double MC_u_reco = h_universe[u]->GetBinContent(reco_bin); //Total Number of MC events in Reco Bin
	nu_universe[reco_bin-1][u] = MC_u_reco + EXT_reco + Dirt_reco;  
      }
    }
  }
