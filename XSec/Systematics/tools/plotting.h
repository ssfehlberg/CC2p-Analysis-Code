void plot_multisims(const char* variable, const char* title, std::vector<TH1D*> hist, TH1D* hist_CV, double ymin, double ymax, int num_universes, const char* directory){
  ...
  //Draw One Multisim (primarily for legend purposes
  hist[0]->Draw("hist");
  ...
  hist[0]->SetYTitle("Differential Cross-Section [10^{-38} cm^{2} / Argon]"); //Y title
  ...
  legend->AddEntry(hist_CV,"Central Value","l");
  legend->AddEntry(hist[num_universes-1],"Multisim Universe","l");
...
void plot_covariance(const char* variable, const char* title, TCanvas* canv, TH2D* hist, TH1D* hist_CV, const char* directory){
...
void plot_error(const char* variable, const char* title, TCanvas* canv, TH1D* hist_error, const char* directory){
...
void plot_fractional_error(const char* variable, const char* title, TCanvas* canv, TH1D* hist_error, const char* directory){
...
TH1D* total_error(const char* name,std::vector<TH1D*> hist){
  ...
  for(int i=1; i < nbins+1; i++){
    ...
    double total_error = std::sqrt(sum + std::pow(0.02,2) + std::pow(0.01,2)); //adding the flux and number of targets uncertainty
...
void plot_GENIE_total_error(const char* name,TCanvas* canv, bool flip_legend,TLegend* legend,std::vector<TH1D*> hist,const char* title, TH1D* h_total){
...
void plot_total_error(const char* name,TCanvas* canv, bool flip_legend,TLegend* legend,std::vector<TH1D*> hist,const char* title, TH1D* h_total){
